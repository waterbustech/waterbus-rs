...
use crate::{
    ...
    models::{
        ...
        input_params::{IceCandidate, JoinRoomParams, JoinRoomResponse, SubscribeParams},
...
impl Media {
    ...
    pub fn forward_media_to_subscribers(&self, media_data: MediaData) {
        for subscriber in self.subscribers.iter() {
            if let Some(writer) = subscriber.rtc.writer(media_data.mid) {
                if let Err(e) = writer.write(media_data.time, media_data.pt, &media_data.data) {
                    warn!(
    ...
    pub fn handle_keyframe_request(&self, req: KeyframeRequest) {
        ...
        if let Some(track_info) = self.tracks.get(&req.mid) {
            ...
            for subscriber in self.subscribers.iter() {
                if let Some(writer) = subscriber.rtc.writer(req.mid) {
...
impl Subscriber {
    pub fn new(participant_id: String, rtc: Arc<Rtc>) -> Self {
        Self {
            participant_id,
            rtc,
